    ...
    for ids,bond in enumerate (bond_list):
        ...
        for k in range (0,(3*N)-6):
            ...
            DELTA_i_j=DELTA_i_j+   (   (1/math.sqrt(w.real[k]))* (
                                        (traj4.xyz[0,i,0]-traj4.xyz[0,j,0])*((v.real[k][3*i]/(math.sqrt(mass_weights[i])))-(v.real[k][3*j]/(math.sqrt(mass_weights[j]))))+
                                        (traj4.xyz[0,i,1]-traj4.xyz[0,j,1])*((v.real[k][3*i+1]/(math.sqrt(mass_weights[i])))-(v.real[k][3*j+1]/(math.sqrt(mass_weights[j]))))+
                                        (traj4.xyz[0,i,2]-traj4.xyz[0,j,2])*((v.real[k][3*i+2]/(math.sqrt(mass_weights[i])))-(v.real[k][3*j+2]/(math.sqrt(mass_weights[j]))))
                                                                  )    
                                    )**2 
...
#         fluctuation_NMA[i][j]=kB*T*DELTA_i_j                    
    ...
    sq_error=np.zeros((len(bond_list)))
    for ids,bond in enumerate (bond_list):
        i,j=int(bond[0])-1,int(bond[1])-1      

        sq_error[ids]=((fluctuation_NMA[ids]-fluctuation_MD[ids]))**2   # minimize the sum of square of error 
...
def force_constants(N,T,ALPHA,beta,max_itr,bond_list,fluctuation_MD,run,tolerance,path,flag_identical_bonds,flag_automate_ALPHA_beta_values,iterative_method,system,traj4,box_size,mass_weights):
    ...
    for iteration in range(0,max_itr):
        ...
        for ids,bond in enumerate (bond_list):
            ...
            bond_list_incre[ids][2]=bond_list[ids][2]+ALPHA
            bond_list_incre[ids][3]=bond_list[ids][3]
            #k_sp_incre[j][i]=k_sp_incre[i][j]

        #to keep the kij_zeros as zero if they are zero once
        ...
        error_ksp=NMA(N,bond_list,fluctuation_MD,T,traj4,mass_weights)
        print("NMA_calculation2")
        error_ksp_incre=NMA(N,bond_list_incre,fluctuation_MD,T,traj4,mass_weights)   
        ...
        for ids,bond in enumerate (bond_list):
            ...
            if iterative_method=='NR_method':
                K_new[ids]=bond_list[ids][2]-(ALPHA*beta*error_ksp[ids])/(error_ksp_incre[ids]-error_ksp[ids])                    # Newton Raphson like                       
            if iterative_method=='Chu_and_Voth_method':                        

                K_new[ids]=bond_list[ids][2]+ALPHA*math.sqrt(error_ksp[ids])  #Chu and Voth 
            if iterative_method=='Lyman_method':                           
                K_value=1/bond_list[ids][2]-ALPHA*math.sqrt(error_ksp[ids]) #Lyman et al. 2008                                                         #Lyman et al. 2008
                K_new[ids]=1/K_value #Lyman et al. 2008
