...
from func_force_field_determination_nr import force_constants_nma   
path=sys.argv[1]
...
#traj2=traj2[::100]
#traj2=traj2.superpose(traj2, frame=0, atom_indices=None, ref_atom_indices=None, parallel=True)
...
max_itr= parameters["max_itr"]          ################## INPUT ##########################
...
method=parameters["method"]  ##################################################### FOR CG_HENM LAMMPS INPUT FILE ##########################
...
if initial_guess=="uniform":
    ...
    # IF WANT TO USE A UNIFORM INITIAL GUESS FOR THE EPSILON
        bond_list[ids][2]=0.01 


if initial_guess=="boltzmann_inversion":

    # INVERSE BOLTZMANN FOR INITIAL GUESS 
    bond_list=func_boltzmann_inversion.boltz_inver_initial_guess(bo_md,SIG_md,bond_list,T,path)
...
if method =="jacobian":
    bond_list=force_constant_jacobian(bond_list,N,fluctuation_MD,traj4,T,mass_weights,20)
